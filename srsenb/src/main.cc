...
#define ENB_PORT 2222
#define ENB_IP "127.0.0.2"
...
int main(int argc, char* argv[])
{
  ...
  // create socket
  int enb_sock = socket(AF_INET, SOCK_DGRAM, 0);
  // bind  address
  ...
  bind(enb_sock, (struct sockaddr*)&enb_addr, sizeof(enb_addr));
  // send data
  ...
  // receive data from EPC
    ...
    memset(&enb_addr, 0, sizeof(enb_addr));
    enb_addr.sin_family      = AF_INET;
    enb_addr.sin_addr.s_addr = inet_addr(recv_ip);
    enb_addr.sin_port        = htons(recv_port);
    // bind address
    bind(rec_sock, (struct sockaddr*)&enb_addr, sizeof(enb_addr));
    */
  // sert from address
  ...
  while (running) {
    ...
    if (send_interval == 100) {
      /*
      ssize_t send_id =sendto(enb_sock, send_data, strlen(send_data), 0, (struct sockaddr*)&route_addr,
      sizeof(route_addr)); printf("-----------------------------sending-----------------------------\n"); printf("index:
      %d\n", num); printf("my ip:               %s:%d\n", inet_ntoa(enb_addr.sin_addr), ntohs(enb_addr.sin_port));
      ...
      ssize_t     rec_id = recvfrom(enb_sock, buffer, sizeof(buffer), 0, (struct sockaddr*)&from_addr, &len);
