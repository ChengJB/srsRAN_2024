  ...
  while (m_running) {
    ...
    if (n == -1) {
      ...
      if (FD_ISSET(s1mme, &m_set)) {
        ...
        std::string s = reinterpret_cast<char*>(pdu->msg);
        printf("s1-mme receieve msg : %s \n", s.c_str());
        Out(sri.sinfo_stream);
        Out(sri.sinfo_ssn);
        Out(sri.sinfo_flags);
        Out(ntohl(sri.sinfo_ppid));
        Out(sri.sinfo_context);
        Out(sri.sinfo_timetolive);
        Out(ntohl(sri.sinfo_tsn));
        Out(sri.sinfo_cumtsn);
        Out(sri.sinfo_assoc_id);
        printf("-----------------------over----------------------\n");
