...
bool s1ap::connect_mme()
{
  ...
  printf("args.s1c_bind_addr.c_str(): %s, args.s1c_bind_port: %d", args.s1c_bind_addr.c_str(), args.s1c_bind_port);
  // Bind socket
  ...
  printf("args.mme_addr.c_str(): %s, MME_PORT: %d", args.mme_addr.c_str(), MME_PORT);
...
bool s1ap::sctp_send_s1ap_pdu(const asn1::s1ap::s1ap_pdu_c& tx_pdu, uint32_t rnti, const char* procedure_name)
{
  ...
  printf("sctp_sendmsg in s1ap::sctp_send_s1ap_pdu ==== streamid%d ==== PPID: %d\n",streamid, PPID);
