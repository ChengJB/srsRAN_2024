...
bool s1ap::connect_mme()
{
  ...
  // Bind socket
  printf("enb to epc bind : address %s and port %d \n", args.s1c_bind_addr.c_str(), args.s1c_bind_port);
  ...
  printf("enb to epc address %s and port %d \n", args.mme_addr.c_str(), MME_PORT);
...
bool s1ap::sctp_send_s1ap_pdu(const asn1::s1ap::s1ap_pdu_c& tx_pdu, uint32_t rnti, const char* procedure_name)
{
  ...
  printf("sctp_sendmsg in sctp_send_s1ap_pdu() \n");
